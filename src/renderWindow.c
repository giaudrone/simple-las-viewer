...
modelBoundingBox
createBoundingBox (LASFHeader header)
{
...
void
renderWindow (PointDataRecord *records, LASFHeader header, int noiseFlag)
{
  glfwInit ();
  ...
  if (noiseFlag)
    {
      ...
      for (size_t i = header.numPointRecords * 0.99f;
           i < header.numPointRecords; i++)
        {
          if (points[i].z - points[i - 1].z > 10)
            {
  ...
  glBufferData (GL_ARRAY_BUFFER, sizeof (data) * header.numPointRecords,
                points, GL_STATIC_DRAW);

  glVertexAttribPointer (0, 3, GL_FLOAT, GL_FALSE, sizeof (data), (void *)0);
  glEnableVertexAttribArray (0);

  glVertexAttribPointer (1, 1, GL_FLOAT, GL_FALSE, sizeof (data),
                         (void *)(3 * sizeof (float)));
  ...
  while (!glfwWindowShouldClose (window))
    {
      ...
      glm_perspective (glm_rad (fov), (float)800 / (float)600, 0.1f,
                       renderDistance, proj);
      ...
      glm_rotate_at (model, (vec3){ 0.0f, 0.0f, 0.0f }, glm_rad (-90.0f),
                     (vec3){ 1.0f, 0.0f, 0.0f });
      glm_rotate_at (model, (vec3){ 0.0f, 0.0f, 0.0f }, glm_rad (180.0f),
                     (vec3){ 0.0f, 0.0f, 1.0f });
...
mouse_callback (GLFWwindow *window, double xpos, double ypos)
{
  ...
  vec3 direction
      = { cos (glm_rad (yaw)) * cos (glm_rad (pitch)), sin (glm_rad (pitch)),
